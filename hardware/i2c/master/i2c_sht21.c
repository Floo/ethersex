...
uint16_t i2c_sht21_read_reg(uint8_t reg, uint8_t res)
{
	...
	if(i2c_sht21_set_res(res) != 0) return 0xFFFF;
...
uint8_t i2c_sht_set_res(uint8_t value)
{
	uint8_t ret = 0xFF;
	uint8_t tmp;
...
uint8_t i2c_sht21_read(char *strData, uint8_t type)
{
	...
	if(type == SHT21_TEMP)
	{
		data32 = (int32)i2c_sht21_read_reg(SHT21_TRIGGER_T, SHT21_UREG_RH8_T12);
		if(data32 == 0xFFFF) return SHT21_ERROR;
		data32 = SHT21_TEMP_C2 * data32 - SHT21_TEMP_C1;
	}else{
		data32 = (int32)i2c_sht21_read_reg(SHT21_TRIGGER_RH, SHT21_UREG_RH12_T14);
		if(data32 == 0xFFFF) return SHT21_ERROR;
		data32 = SHT21_HUMID_C2 * data32 - SHT21_HUMID_C1;
	}
	data16 = (int16)(data32 >> 16); //value with 2 digits after decimal point
	if (data16 > 0) //round()
	{
		data16 += 5;
	}else{
